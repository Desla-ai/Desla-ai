...
function isMonthlyMode(v: unknown) {
  return String(v ?? "") === "MONTHLY"
}
...
export async function GET(req: Request) {
  ...
  const monthlyPayoutEnabled = isMonthlyMode(site.default_settlement_mode)
...
export async function PUT(req: Request) {
  ...
  // ✅ 명시적으로 boolean을 받도록(실수로 undefined 들어오면 바로 400)
  if (typeof body?.monthlyPayoutEnabled !== "boolean") {
    return jsonError("monthlyPayoutEnabled(boolean) is required", 400)
  }
  ...
  return NextResponse.json({
    ok: true,
    siteId: data.id,
    monthlyPayoutEnabled: isMonthlyMode(data.default_settlement_mode),
